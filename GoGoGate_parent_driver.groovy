    ...
    children.each { child->
		...
		if (child.deviceNetworkId.contains("temp")) {
			...
			try {
                ...
                if (!temp || !battery) {
                    log("Sensor for ${currentDoor} indicates temp as ${temp} and the battery level as ${battery}. Gogogate2 has not contacted sensor yet.","error")
                } else {
...
def getSensorInfo(int sensor) {
    if (testCookie()) {
		...
		if (resp) {
            ...
            if (!temp && !battery) {
			    log("Unable to get sensor data. Possible battery problem?","warn")
            }
            if (temp > -40 && temp < 60) {
    ...
    if (testOptions()) {
        ...
        try{
            ...
            if (e.toString().contains("No route to host")) {
                log("No route to host, please check your IP address.","error")
                state.cookie = null
                state.configured = false
            } else {
                log("GoGoGate 2 returned: ${e}","error")
